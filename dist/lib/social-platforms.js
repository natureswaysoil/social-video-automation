...
async function fetchBasicMetrics(videoIds) {
    const metrics = {
    ...
    if (videoIds.instagramId && process.env.INSTAGRAM_ACCESS_TOKEN) {
        try {
            ...
            const response = await axios_1.default.get(`https://${host}/${apiVersion}/${videoIds.instagramId}?fields=like_count,comments_count,reach,video_view_count`, {
    ...
    if (videoIds.facebookId) {
        try {
            ...
            if (accessToken) {
                ...
                const summary = await axios_1.default.get(`https://${host}/${apiVersion}/${videoIds.facebookId}?fields=reactions.summary(true),comments.summary(true),views`, {

...
exports.composeVerticalAd = composeVerticalAd;
// @ts-nocheck
...
function probeDuration(file) {
    try {
        const out = (0, child_process_1.execSync)(`ffprobe -v error -show_entries format=duration -of csv=p=0 "${file}"`).toString().trim();
        const n = Number(out);
 ...
 */
function makeSceneClip(file, index, seconds, kind, outputDir) {
    const duration = Math.max(3, Number(seconds || 5));
    const frames = Math.ceil(duration * 30);
    const clip = path_1.default.resolve(outputDir, `scene-${Date.now()}-${index}.mp4`);
 ...
 */
async function composeVerticalAd(input) {
    ...
    let scenes = Array.isArray(input.scenes) ? input.scenes.filter((s) => s && s.file) : [];
    if (!scenes.length) {
        const files = input.sceneFiles || [];
        if (!files.length)
            throw new Error('No scene files provided to compositor');
    ...
    const hasProductScene = scenes.some((s) => s.kind === 'product');
    const sceneClips = scenes.map((s, i) => makeSceneClip(s.file, i, s.seconds, s.kind, outputDir));
    // ---- stitch ----
    const concatList = path_1.default.resolve(outputDir, `concat-${Date.now()}.txt`);
    fs_1.default.writeFileSync(concatList, sceneClips.map((f) => `file '${f.replace(/'/g, "'\\''")}'`).join('\n'), 'utf8');
    let working = path_1.default.resolve(outputDir, `stitched-${Date.now()}.mp4`);
    run([
        'ffmpeg -y -loglevel error',
        '-f concat -safe 0',
        `-i "${concatList}"`,
    ...
    if (input.productImage && !hasProductScene) {
        const out = path_1.default.resolve(outputDir, `product-${Date.now()}.mp4`);
        run([
            'ffmpeg -y -loglevel error',
            `-i "${working}"`,
    ...
    if (input.overlayText) {
        const out = path_1.default.resolve(outputDir, `overlay-${Date.now()}.mp4`);
        run([
            'ffmpeg -y -loglevel error',
            `-i "${working}"`,
    ...
    if (input.captionText) {
        const out = path_1.default.resolve(outputDir, `caption-${Date.now()}.mp4`);
        run([
            'ffmpeg -y -loglevel error',
            `-i "${working}"`,
    ...
    if (input.voiceoverFile && fs_1.default.existsSync(input.voiceoverFile)) {
        const audioDur = probeDuration(input.voiceoverFile);
        const videoDur = probeDuration(working);
        const out = path_1.default.resolve(outputDir, `voiced-${Date.now()}.mp4`);
    ...
    const finalOutput = path_1.default.resolve(outputDir, `${(0, video_utils_1.safeFileName)(input.outputName || 'vertical-ad', 'mp4')}`);
    fs_1.default.copyFileSync(working, finalOutput);
    return finalOutput;
}

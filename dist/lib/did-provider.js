...
exports.didPresenter = didPresenter;
exports.didVoice = didVoice;
exports.createDidVideo = createDidVideo;
exports.pollDidVideo = pollDidVideo;
// @ts-nocheck
...
function authHeaders() {
    ...
    if (!apiKey)
        throw new Error('Missing DID_API_KEY. Google Secret Manager alias DiD is supported by validate-config.');
    return {
...
function productGroup(product) {
    ...
    if (/dog|pet|urine|odor|kennel|turf/.test(text))
        return 'pet';
    if (/pasture|hay|field|farm|acre|cattle|horse/.test(text))
        return 'pasture';
    if (/compost|worm|biochar|raised|garden|vegetable|flower|potted/.test(text))
        return 'garden';
    if (/lawn|grass|turf|soil recovery|humic|fulvic|kelp/.test(text))
        return 'lawn';
    return 'default';
}
function didPresenter(profile, product) {
...
function didVoice(profile, product) {
    const group = productGroup(product);
    return profile.didVoiceId || process.env.DID_VOICE_ID || VOICE_MAP[group] || VOICE_MAP.default;
}
async function createDidVideo(product, scenePlan, profile) {
    ...
    const script = scenePlan.fullVoiceover || (scenePlan.scenes || []).map((s) => s.voiceover).join(' ');
    const body = {
        script: {
            type: 'text',
            input: script,
    ...
    if (presenter.type === 'source_url')
        body.source_url = presenter.value;
    else
        body.presenter_id = presenter.value;
    try {
        const response = await axios_1.default.post(`${endpoint}/talks`, body, {
            headers: authHeaders(),
            timeout: 120000
        });
        const id = response.data?.id;
        if (!id)
            throw new Error(`D-ID did not return talk id: ${JSON.stringify(response.data)}`);
...
async function pollDidVideo(id) {
    const endpoint = process.env.DID_API_ENDPOINT || 'https://api.d-id.com';
    ...
    while (Date.now() - start < timeoutMs) {
        try {
            const response = await axios_1.default.get(`${endpoint}/talks/${id}`, {
                headers: authHeaders(),
                timeout: 60000
            });
            ...
            if ((status === 'done' || status === 'completed') && data.result_url)
            ...
            if (error.message?.startsWith('D-ID failed:'))
                throw error;

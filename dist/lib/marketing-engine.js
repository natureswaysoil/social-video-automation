...
exports.amazonAttributionUrl = amazonAttributionUrl;
exports.recordPerformance = recordPerformance;
exports.pickABVariant = pickABVariant;
exports.generateTrendAwareHooks = generateTrendAwareHooks;
exports.updateTrendsManually = updateTrendsManually;
...
const TRENDS_FILE = path_1.default.resolve(ROOT, process.env.TRENDS_FILE || 'data/trends.json');
function amazonAttributionUrl(product, campaign = 'social_video') {
    ...
    const content = encodeURIComponent((0, video_utils_1.slugify)(product.name || product.id || 'product'));
...
function pickABVariant(product) {
    const analytics = (0, video_utils_1.readJson)(ANALYTICS_FILE, { hooks: {}, videos: [] });
    const candidates = [
        'problem_first',
        'before_after',
    ...
    for (const video of analytics.videos || []) {
        if (video.productId === product.id && video.variant && counts[video.variant] !== undefined)
            counts[video.variant]++;
    }
    const variantScores = Object.fromEntries(candidates.map((c) => [c, 0]));
    for (const video of analytics.videos || []) {
        if (video.productId !== product.id || !video.variant || variantScores[video.variant] === undefined)
            continue;
...
async function generateTrendAwareHooks(product, profile) {
    const trends = (0, video_utils_1.readJson)(TRENDS_FILE, { trends: [] });

...
import { runFfmpeg, escapeFilterText } from './ffmpeg'

export function ffmpegEscapeText(text: string): string {
  // Filter-syntax escaping (also escapes commas for use mid-filtergraph).
  return escapeFilterText(text).replace(/,/g, '\\,')
}

export function animatedCaptionFilter(text: string, style = 'hook'): string {
...
export function ctaFilter(text: string): string {
  const safe = ffmpegEscapeText(text)
...
export function circlePipFilter(size = 320): string {
...
export function buildSplitScreen(beforeFile: string, afterFile: string, outputFile: string): string {
...
export function createProductCutoutPlaceholder(productImage: string, outputDir: string): string {
  ...
  runFfmpeg(['-y', '-i', productImage, '-vf', 'scale=520:-1', output])
  return output
}

export function mixSoundtrack(videoFile: string, musicFile: string, outputFile: string): string {

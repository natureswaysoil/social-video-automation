...
import { runFfmpeg, escapeFilterText, probeDuration } from './ffmpeg'

export type SceneKind = 'product' | 'photo' | 'video'
...
export interface ComposeVerticalAdInput {
  outputName?: string
 ...
 */
function makeSceneClip(file: string, index: number, seconds: number, kind: string, outputDir: string): string {
  ...
  if (resolvedKind === 'photo') {
    ...
    const vf = `scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,${move}:d=${frames}:s=1080x1920:fps=30,format=yuv420p`
    runFfmpeg([
      '-y', '-loglevel', 'error',
      '-loop', '1',
      '-i', file,
 ...
 */
export async function composeVerticalAd(input: ComposeVerticalAdInput): Promise<string> {
  const outputDir = path.resolve(process.cwd(), 'output')
  ensureDir(outputDir)

  // Normalise to a scenes[] array with explicit kinds.
  let scenes: CompositorScene[] = Array.isArray(input.scenes) ? input.scenes.filter((s) => s && s.file) : []
  if (!scenes.length) {
  ...
  const sceneClips = scenes.map((s, i) =>
    makeSceneClip(s.file, i, Number(s.seconds || 5), s.kind || '', outputDir))
  ...
  if (input.overlayText) {
    ...
    runFfmpeg([
      '-y', '-loglevel', 'error',
      '-i', working,
      '-vf', `drawtext=text='${escapeFilterText(input.overlayText)}':fontcolor=white:fontsize=42:borderw=4:bordercolor=black:x=40:y=90:box=1:boxcolor=black@0.35:boxborderw=18`,
  ...
  if (input.captionText) {
    ...
    runFfmpeg([
      '-y', '-loglevel', 'error',
      '-i', working,
      '-vf', `drawtext=text='${escapeFilterText(input.captionText)}':fontcolor=white:fontsize=52:borderw=4:bordercolor=black:x=(w-text_w)/2:y=h-260:box=1:boxcolor=black@0.25:boxborderw=18`,
      '-c:v', 'libx264', '-preset', 'veryfast', '-pix_fmt', 'yuv420p',
      out
    ])
    working = out

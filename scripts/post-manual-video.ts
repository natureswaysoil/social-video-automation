...
function enabledPlatforms(): string[] {
  return Array.from(new Set(
    String(process.env.ENABLE_PLATFORMS || 'youtube,facebook')
      .toLowerCase()
      .split(',')
      .map((item) => item.trim())
      .filter(Boolean)
...
async function postToYouTube(videoUrl: string, title: string, description: string): Promise<string> {
  ...
  const body = (await axios.get(videoUrl, { responseType: 'stream', timeout: 120000 })).data

  const upload = await youtube.videos.insert({
    part: ['snippet', 'status'],
    requestBody: {
      snippet: { title: title.slice(0, 95), description, categoryId: '22' },
      status: { privacyStatus: (process.env.YT_PRIVACY_STATUS || process.env.YOUTUBE_PRIVACY_STATUS || 'public') as any },
    },
    media: { body },
  })

  const id = upload.data.id || ''
  if (!id) throw new Error('YouTube upload did not return an id')
  return id
}

async function postToFacebook(videoUrl: string, caption: string): Promise<string> {
  ...
  if (!accessToken || !pageId) throw new Error('Missing Facebook credentials')

  const response = await axios.post(
    `https://graph.facebook.com/${process.env.FACEBOOK_API_VERSION || 'v20.0'}/${pageId}/videos`,
    { file_url: videoUrl, description: caption, published: true },
    { headers: { Authorization: `Bearer ${accessToken}` }, timeout: 120000 }
  )

  const id = response.data?.id || ''
  if (!id) throw new Error('Facebook upload did not return an id')
  return id
}

async function main() {
  const videoUrl = String(process.env.MANUAL_VIDEO_URL || '').trim()
  if (!/^https:\/\//i.test(videoUrl)) throw new Error('MANUAL_VIDEO_URL must be a public HTTPS video URL')
  ...
  if (posted === 0) throw new Error('No manual video posts succeeded')
